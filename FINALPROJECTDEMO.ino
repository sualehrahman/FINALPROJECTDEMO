...
//Variable intiation for character stats I used struct because it groups variables in one place| Got the struct idea from https://www.w3schools.com/cpp/cpp_structs.asp 
struct Character {
  const char* name; //
  int health;
  int minAttackPower;
...
//function for printing players health value
void printStatus(Character& character) {
  Serial.print(character.name);
  Serial.print(" - Health: ");
  Serial.println(character.health);
}
//function for the random attack and giving an attack value to the player
...
//if defender health is less than or equal to 0
  if (defender.health < 0) defender.health = 0;

  printStatus(defender);
// function for the sound when you get hit
...
//if player hp is below or equal to 0 end the game change the leds to red and lose the game also plays a defeat theme
//if boss hp is below or equal to 0 end the game change the leds to green and win the game also plays a victory theme
